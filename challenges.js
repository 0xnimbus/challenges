...
function formatWithPadding(integer, character, length) {
  var result = integer.toFixed(0);
...
function hammingDistance(num1, num2) {
  if (num1.length !== num2.length) return NaN;
  var count = 0;
...
function mergeObjects(targets, ...objects) {
  objects.forEach(function(object) {
    for(var key in object) {
      targets[key] = object[key];
    }
  });
  return targets;
}
...
function findHighestPriced(array) {
  ...
  array.forEach(function(items) {
    if (items.price > highestPrice) {
      highestPrice = items.price;
      resultObj = items;
    }
  });
  return resultObj;
...
function mapArray(array, temp) {
  var newArr = [];
  array.forEach(function(el, idx) {
    newArr.push( temp(el, idx) );
  });
  return newArr;
}
...
function reduceArray(array, temp, initAcc) {
  var acc = initAcc;
  array.forEach(function(el, idx) {
    acc = temp(acc, el, idx);
  });
...
function intersection(temp1, temp2) {
  var result = [];
  var _temp2 = [...temp2];
  temp1.forEach(val => {
    var idx = _temp2.indexOf(val);
    if (idx > -1) result.push(_temp2.splice(idx, 1)[0]);
  });
  return result;
}
...
function isWinningTicket(tic){
  var winner = true;
  for (var i = 0; i < tic.length; i++) {
    var charFromNumber = String.fromCharCode(tic[i][1]);
    if (!tic[i][0].includes(charFromNumber)) {
      winner = false;
      break;
    }
  }
...
function getNumForIP(num) {
  var chunks = num.split('.').reverse();
  var sum = 0;
...
function countTheBits(num) {
  return num.toString(2).split('').filter(t => t === '1').length;
}
...
function addChecker(int, sum) {
  var result = false;
  for (i = 0; i < int.length - 1; i++) {
    for (j = i + 1; j < int.length; j++) {
      if (int[i] + int[j] === sum) return true; 
    }
  }
  return result;
}
